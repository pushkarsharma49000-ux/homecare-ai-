...
export async function GET(request: Request) {
  ...
  const { data, error } = await supabase.from('service_requests').select('id, request_number, issue, status, created_at, appliances(brand, model, appliance_type), appointments(id, scheduled_start, scheduled_end, status, calendar_event_id)').eq('customer_id', customer.id).order('created_at', { ascending: false });
